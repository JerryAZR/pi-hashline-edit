...
- **Two-tier stale‑anchor resolution.** When anchors don't match the live file, the tool tries two strategies in order: (1) exact match — some edits may still be valid against current content; (2) snapshot merge — if the anchors match the most recent `read` snapshot but not the live file, a 3‑way merge rebases the edits. Any edit that survives both tiers is applied; any that fails rejects the entire request with `[E_STALE_ANCHOR]` and asks for a fresh `read`.
